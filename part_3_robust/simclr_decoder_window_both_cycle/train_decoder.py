...
def train_encoder_decoder(model, num_epochs, trainloader, testloader, optimizer, scheduler, device, loss_fn_contrastive, cycle_loss, loss_fn_reconstruct, model_save_path="barlow_twins.pth", alpha=0.5):
    alpha_start = 1.0
    alpha_end = 0.0
    k = 0.01
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            Z, recon_X =  model(X_augment)
            Z_prime, recon_X_prime =  model(X_prime_augment)
            Z_prime_2, _ = model(X_prime_2)

            _, recon_masked = model(X_masked)
            loss_cycle = cycle_loss(Z_prime - 2 * Z + Z_prime_2, torch.zeros_like(Z))
            loss_contrastive =  loss_fn_contrastive(Z, Z_prime) + loss_fn_contrastive(Z, Z_prime_2) + loss_cycle
