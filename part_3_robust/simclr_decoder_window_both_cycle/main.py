...
def main():
    ...
    train_encoder_decoder(model=model_decoder, num_epochs=num_epochs, trainloader=trainloader, testloader=validloader, optimizer=optimizer, scheduler=scheduler, device=DEVICE, loss_fn_contrastive=loss_fn_contrastive, cycle_loss=cycle_loss, loss_fn_reconstruct=loss_fn_reconstruct, model_save_path='simclr_decoder.pth', alpha=0.1)
