...
class WeatherBenchDataset(Dataset):
    def __init__(
        self, data, window=30, gap=1000, augment_sample_random_mask=0.7
    ):
        self.data = data
    ...
    def _hard_neg_idx(self, t):
        candidates = [
            i
            for i in range(t - self.window, t + self.window + 1)
            if i != t and 0 <= i < len(self.data)
        ]
        return np.random.choice(candidates)

    def _soft_neg_idx(self, t):
        candidates = list(range(0, t - self.gap)) + list(
            range(t + self.gap, len(self.data))
        )
        return np.random.choice(candidates)
