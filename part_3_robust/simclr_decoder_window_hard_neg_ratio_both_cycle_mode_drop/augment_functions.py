...
def augment_sample_random_mask(sample, channel, mask_prob_low=0.5, mask_prob_high=0.9, p=0.2):
    channel_to_drop = None
    if random.random() < p:
        C, H, W = sample.shape
        if channel is not None:
            if isinstance(channel, list):
                available_channels = [i for i in range(C) if i not in channel]
            else:
                available_channels = [i for i in range(C) if i != channel]
            channel_to_drop = random.choice(available_channels)
        else:
            channel_to_drop = random.randint(0, C - 1)
        sample = sample.clone()
        sample[channel_to_drop] = 0
