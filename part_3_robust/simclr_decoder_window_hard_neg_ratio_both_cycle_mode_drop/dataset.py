...
class WeatherBenchDataset(Dataset):
    ...
    def _create_sample(self, idx):   
        ...
        x_prime, channel_2 = augment_sample_random_mask(X_prime, channel=channel, mask_prob_low=self.mask_prob_low, mask_prob_high=self.mask_prob_high)
        x_prime_2, _ = augment_sample_random_mask(X_prime_2, channel=[channel, channel_2], mask_prob_low=self.mask_prob_low, mask_prob_high=self.mask_prob_high)
        return x, x_prime, x_prime_2, X, X_masked


    def __getitem__(self, idx):
        x, x_prime, x_prime_2, X, X_masked = self._create_sample(idx)
        hard_idx = self._hard_neg_idx(idx)
        soft_idx = self._soft_neg_idx(idx)
        x_soft, x_prime_soft, x_prime_2_soft, X_soft, X_masked_soft = self._create_sample(soft_idx)
        x_hard, x_prime_hard, x_prime_2_hard, X_hard, X_masked_hard = self._create_sample(hard_idx)
        return torch.stack([x, x_soft, x_hard]), torch.stack([x_prime, x_prime_soft, x_prime_hard]), torch.stack([x_prime_2, x_prime_2_soft, x_prime_2_hard]), torch.stack([X, X_soft, X_hard]), torch.stack([X_masked, X_masked_soft, X_masked_hard])
