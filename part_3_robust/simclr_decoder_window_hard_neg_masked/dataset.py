...
class WeatherBenchDataset(Dataset):
    ...
    def _create_sample(self, idx):
        ...
        x_prime_idx = augment_sample_random_mask(
    ...
    def __getitem__(self, idx):
        ...
        x_soft, x_prime_soft, x_prime_soft_idx, X_soft, X_masked_soft = (
            self._create_sample(soft_idx)
        )
        x_hard, x_prime_hard, x_prime_hard_idx, X_hard, X_masked_hard = (
            self._create_sample(hard_idx)
        )
        return (
            torch.stack([x, x_soft, x_hard]),
            torch.stack([x_prime, x_prime_soft, x_prime_hard]),
            torch.stack([x_prime_idx, x_prime_soft_idx, x_prime_hard_idx]),
            torch.stack([X, X_soft, X_hard]),
