...
class WeatherBenchDataset(Dataset):
    ...
    def _hard_neg_idx(self, t):
        ...
        """
        exclude = set(range(t - 2 * self.max_delta_t, t + 2 * self.max_delta_t + 1))
        candidates = [
            i
            for i in range(
            ...
            if i not in exclude and 0 <= i < len(self.data) - 2 * self.max_delta_t
        ]
    ...
    def _soft_neg_idx(self, t):
        ...
        candidates = (
            ...
            + list(range(t + self.gap, len(self.data) - 2 * self.max_delta_t))
        )
        return np.random.choice(candidates)
    ...
    def __getitem__(self, idx):
        ...
        """
        x, x_prime, x_prime_2, X, X_masked, X_prime, X_prime_2, aug_idx = self._create_sample(idx)
        hard_idx = self._hard_neg_idx(idx)
        soft_idx = self._soft_neg_idx(idx)

        x_soft, x_prime_soft, x_prime_2_soft, X_soft, X_masked_soft, X_prime_soft, X_prime_2_soft, aug_idx_soft = self._create_sample(soft_idx)
        x_hard, x_prime_hard, x_prime_2_hard, X_hard, X_masked_hard, X_prime_hard, X_prime_2_hard, aug_idx_hard = self._create_sample(hard_idx)
        ...
        return (
            ...
            torch.stack([X_prime, X_prime_soft, X_prime_hard]),
            torch.stack([X_prime_2, X_prime_2_soft, X_prime_2_hard]),
            np.stack([aug_idx, aug_idx_soft, aug_idx_hard]),
        )
