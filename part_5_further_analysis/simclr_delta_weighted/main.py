...
def main():
    ...
    train_dataset = WeatherBenchDataset(
        data=train_data, mask_prob_low=0.5, mask_prob_high=0.9, max_delta_t=5
    )
    valid_dataset = WeatherBenchDataset(
        data=valid_data, mask_prob_low=0.5, mask_prob_high=0.9, max_delta_t=5
    )
    ...
    loss_fn_contrastive = SelfSupervisedLoss(
        NoReducuctionNTXentLoss(temperature=0.3)
    )
