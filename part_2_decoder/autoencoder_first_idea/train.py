...
def train_autoencoder(model, num_epochs, trainloader, testloader, optimizer, scheduler, device, loss_fn, alpha=0.5, model_save_path="det_autoencoder.pth"):
    for epoch in range(num_epochs):
        ...
        print(f'Epoch: {epoch}, Train Loss = {np.mean(train_loss):.2f}, Train Recon Loss = {np.mean(train_recon_1_loss):.2f}, Train Masked Recon Loss = {np.mean(train_recon_2_loss):.2f}, Valid loss = {np.mean(valid_loss):.2f}, Valid Recon Loss = {np.mean(valid_recon_1_loss):.2f}, Valid Masked Recon Loss = {np.mean(valid_recon_2_loss):.2f}, lr = {lr:.5f}')
