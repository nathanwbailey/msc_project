...
def main():
    ...
    ds_full = xr.open_dataset(
        target_path, engine='zarr', chunks={}, backend_kwargs={'storage_options': {'anon': True}}
    )
    ...
    ds_full = xr.Dataset({
        ...
        'u_component_of_wind': ds_full['u_component_of_wind'].sel(level=850, drop=True),
        'v_component_of_wind': ds_full['v_component_of_wind'].sel(level=850, drop=True),
        'geopotential': ds_full['geopotential'].sel(level=850, drop=True),
        'specific_humidity': ds_full['specific_humidity'].sel(level=850, drop=True),
    })
    ...
    with ProgressBar():
        ...
        torch.save(tensor_data, '/vol/bitbucket/nb324/ERA5_64x32_daily_850.pt')
