...
def replace_bn_with_gn(module, num_groups=8):
    for name, child in module.named_children():
        if isinstance(child, nn.BatchNorm2d):
            ...
            gn = nn.GroupNorm(num_groups=1, num_channels=num_channels)
...
class Decoder(nn.Module):
    def __init__(self, in_channels, initial_kernel, latent_dim, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.latent_dim = latent_dim
        self.project_dim = 128
        self.project_layer = nn.Linear(latent_dim, 1024)
        self.decoder = nn.Sequential(
            DecoderBlock(in_channels=self.project_dim, out_channels=self.project_dim//2), # 4x2 -> 8x4
            DecoderBlock(in_channels=self.project_dim//2, out_channels=self.project_dim//4), # 8x4 -> 16x8
            DecoderBlock(in_channels=self.project_dim//4, out_channels=self.project_dim//8), # 16x8 -> 32x16
            DecoderBlock(in_channels=self.project_dim//8, out_channels=self.project_dim//16), # 32x16 -> 64x32
        )
        self.channel_layer = nn.Conv2d(in_channels=self.project_dim//16, out_channels=in_channels, kernel_size=1, stride=1)
...
class SIMCLRDecoder(nn.Module):
    def __init__(self, in_channels, model, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.model = model
        self.decoder = Decoder(in_channels=in_channels, initial_kernel=(8, 4), latent_dim=1000)
