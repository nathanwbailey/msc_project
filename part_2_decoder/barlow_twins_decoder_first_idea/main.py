...
from model_decoder import BarlowTwinsDecoder, BarlowTwins
...
def main():
    ...
    loss_fn_contrastive = BarlowTwinsLoss(scale_factor=1, lambd=0.01)
    ...
    train_encoder_decoder(model=model_decoder, num_epochs=num_epochs, trainloader=trainloader, testloader=validloader, optimizer=optimizer, scheduler=scheduler, device=DEVICE, loss_fn_contrastive=loss_fn_contrastive, loss_fn_reconstruct=loss_fn_reconstruct, model_save_path='barlow_decoder_fine_tuned.pth', alpha=0.1)

    # model_decoder = torch.load('barlow_decoder_fine_tuned.pth', weights_only=False, map_location=torch.device('cpu'))
