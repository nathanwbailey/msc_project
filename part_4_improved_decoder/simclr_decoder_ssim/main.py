...
def main():
    ...
    loss_fn_ssim = StructuralSimilarityIndexMeasure(data_range=None).to(DEVICE)
    ...
    #downstream_task_lstm(num_epochs=100, data=test_data, encoder_model=model_decoder.model.encoder, latent_dim=1000, context_window=30, target_length=5, stride=1, model_save_path='downstream_model_no_decoder_5.pth')
    ...
    train_decoder(model=model_decoder, num_epochs=200, trainloader=trainloader, testloader=validloader, optimizer=optimizer, scheduler=scheduler, device=DEVICE, loss_fn_reconstruct=loss_fn_reconstruct, loss_fn_ssim=loss_fn_ssim, model_save_path='simclr_decoder_freeze.pth', x_min=x_min, x_max=x_max, mean=mean, std=std)
