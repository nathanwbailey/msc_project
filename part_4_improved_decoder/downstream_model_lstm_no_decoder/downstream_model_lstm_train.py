...
def shuffle_sequence_embeddings(embeddings):
    B, T, D = embeddings.size()
    perm = torch.randperm(T, device=embeddings.device)
    shuffled = embeddings[:, perm, :]
    return shuffled


def train_lstm_model(num_epochs, encoder_model, seq2seq_model, loss_fn, optimizer, scheduler, trainloader, validloader, device, model_save_path='lstm_model.pth', shuffle_embeddings_flag=False):
    def prepare_inputs(input_data, output_data, encoder_model, shuffle_embeddings_flag=False):
        B, T, C, H, W = input_data.shape
        input_data = input_data.reshape(B*T, C, H, W)
        input_encoded_data = encoder_model(input_data)
        if isinstance(input_encoded_data, tuple):
            input_encoded_data = input_encoded_data[0]
        input_encoded_data = input_encoded_data.reshape(B, T, -1)
        B, T, C, H, W = output_data.shape
        output_data = output_data.reshape(B*T, C, H, W)
        output_encoded_data = encoder_model(output_data)
        if isinstance(output_encoded_data, tuple):
            output_encoded_data = output_encoded_data[0]
        output_encoded_data = output_encoded_data.reshape(B, T, -1)


        if shuffle_embeddings_flag:
            input_encoded_data = shuffle_sequence_embeddings(input_encoded_data)
    ...
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            input_encoded_data, output_encoded_data = prepare_inputs(input_data, output_data, encoder_model, shuffle_embeddings_flag=shuffle_embeddings_flag)
