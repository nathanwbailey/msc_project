...
class Block(nn.Module):
    def __init__(
        self, in_channels, out_channels, time_emb_dim, groups=32, up=None
    ):
        super().__init__()
...
class LatentNetwork(nn.Module):
    def __init__(self, latent_dim, time_emb_dim, emb_dim, *args, **kwargs):
        ...
        self.cond_projs_enc = nn.ModuleList(
            [
                nn.Conv2d(
                    in_channels=16, out_channels=32, stride=2, kernel_size=2
                ),
                nn.Conv2d(
                    in_channels=16, out_channels=64, stride=4, kernel_size=4
                ),
                nn.Conv2d(
                    in_channels=16, out_channels=128, stride=4, kernel_size=4
                ),
            ]
        )
        ...
        self.cond_projs_dec = nn.ModuleList(
            [
                nn.Conv2d(
                    in_channels=16, out_channels=128, stride=2, kernel_size=2
                ),
                nn.Conv2d(
                    in_channels=16, out_channels=64, stride=1, kernel_size=1
                ),
            ]
        )

        self.reshape_layer_in = nn.Linear(latent_dim, emb_dim)
        self.initial_layer = ResidualBlock2D(
            in_channels=16, out_channels=32, groups=16
        )
