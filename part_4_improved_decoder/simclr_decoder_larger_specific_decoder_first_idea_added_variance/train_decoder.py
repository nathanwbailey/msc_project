...
def train_encoder_decoder(model, num_epochs, trainloader, testloader, optimizer, scheduler, device, loss_fn_contrastive, loss_fn_reconstruct, cycle_loss, model_save_path="barlow_twins.pth", add_l1=False, add_l2=False, l1_lambda=1e-6, l2_lambda=1e-6):
    ...
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            var_loss = variance_loss(z1_x) + variance_loss(z2_x) + variance_loss(z3_x)
            cov_loss = covariance_loss(z1_x) + covariance_loss(z2_x) + covariance_loss(z3_x)
            ...
            loss_contrastive = loss_fn_contrastive(z1, z2) + loss_fn_contrastive(z1, Z_prime_2) + loss_cycle + loss_contrastive_recon + 1 * var_loss + 1 * cov_loss
