...
def train_transformer_model(
    ...
    def prepare_inputs(input_data, output_data, encoder_model):
        ...
        output_encoded_data = torch.cat(
            (
                input_encoded_data[:, -1, ...].unsqueeze(1),
                output_encoded_data,
            ),
            dim=1,
        )
    ...
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            target_mask = (
                torch.nn.Transformer.generate_square_subsequent_mask(T)
            )
        ...
        print(
            f"Epoch: {epoch}, Train Loss: {np.mean(train_loss):.10f}, Valid Loss: {np.mean(valid_loss):.10f}, LR: {lr}"
        )
