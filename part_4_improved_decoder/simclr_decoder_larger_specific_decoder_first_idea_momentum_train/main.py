...
def main():
    ...
    momentum_model = copy.deepcopy(model_decoder).to(DEVICE)
    for param in momentum_model.parameters():
        param.requires_grad = False
    
    momentum_model.eval()

    train_encoder_decoder(model=model_decoder, momentum_model=momentum_model, num_epochs=num_epochs, trainloader=trainloader, testloader=validloader, optimizer=optimizer, scheduler=scheduler, device=DEVICE, loss_fn_contrastive=loss_fn_contrastive, loss_fn_reconstruct=loss_fn_reconstruct, cycle_loss=cycle_loss, model_save_path='simclr_decoder.pth')

    #model_decoder = torch.load("simclr_decoder.pth", weights_only=False)

    print('Starting Downstream Task')
    downstream_task_lstm(num_epochs=100, data=test_data, encoder_model=momentum_model.model.encoder, latent_dim=1000, context_window=30, target_length=1, stride=1, model_save_path='downstream_model_no_decoder_weight_decay.pth', weight_decay=1e-5)


    optimizer = torch.optim.Adam(momentum_model.decoder.parameters(), lr=learning_rate_decoder, weight_decay=0)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=10, threshold=0.0001)

    print('Training Decoder')

    train_decoder(model=momentum_model, num_epochs=200, trainloader=trainloader, testloader=validloader, optimizer=optimizer, scheduler=scheduler, device=DEVICE, loss_fn_reconstruct=loss_fn_reconstruct, model_save_path='simclr_decoder_freeze.pth')
