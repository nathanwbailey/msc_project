...
def main():
    ...
    trainloader_outer = DataLoader(train_dataset, batch_size=BATCH_SIZE_OUTER, shuffle=True, **loader_kwargs)
    validloader_outer = DataLoader(valid_dataset, batch_size=BATCH_SIZE_OUTER, shuffle=False, **loader_kwargs)
    trainloader_inner = DataLoader(train_dataset, batch_size=BATCH_SIZE_INNER, shuffle=True, **loader_kwargs)
    validloader_inner = DataLoader(valid_dataset, batch_size=BATCH_SIZE_INNER, shuffle=False, **loader_kwargs)
    ...
    optimizer_mae = torch.optim.Adam(model_decoder.parameters(), lr=LEARNING_RATE_DECODER, weight_decay=0)
    optimizer_simclr = torch.optim.Adam(model_decoder.model.parameters(), lr=LEARNING_RATE, weight_decay=0)
    ...
    optimizer = torch.optim.Adam(model_decoder.decoder.parameters(), lr=LEARNING_RATE, weight_decay=0)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=10, threshold=0.0001)
