...
def train_encoder_decoder(model, num_epochs, trainloader, testloader, optimizer, scheduler, device, loss_fn_contrastive, loss_fn_reconstruct, cycle_loss, model_save_path="barlow_twins.pth", add_l1=False, add_l2=False, l1_lambda=1e-6, l2_lambda=1e-6, alpha_cycle=10):
    alpha_start = 1.0
    alpha_end = 0.0
    k = 0.01
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            Z_prime_2, z3_recon, z3_x = model(X_prime_2)
            _, recon_masked, _ = model(X_masked)
            loss_cycle = cycle_loss(z2_x - 2 * z1_x + z3_x, torch.zeros_like(z1_x))
            ...
            loss_contrastive = loss_fn_contrastive(z1, z2) + loss_fn_contrastive(z1, Z_prime_2) + alpha_cycle * loss_cycle + loss_contrastive_recon
