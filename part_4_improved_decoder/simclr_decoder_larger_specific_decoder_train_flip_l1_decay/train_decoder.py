...
def train_decoder(model, num_epochs, trainloader, testloader, optimizer, scheduler, device, loss_fn_reconstruct, model_save_path="simclr.pth", add_l1=False, l1_lambda=1e-5):
...
def train_encoder_decoder(model, num_epochs, trainloader, testloader, optimizer, scheduler, device, loss_fn_contrastive, loss_fn_reconstruct, cycle_loss, model_save_path="barlow_twins.pth", add_l1=False, l1_lambda=1e-5):
    alpha_start = 1.0
    alpha_end = 0.0
    k = 0.01
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            loss_batch = (1-alpha) * loss_contrastive + (alpha) * (loss_recon_X)
            if add_l1:
                l1_norm = sum(p.abs().sum() for p in model.parameters())
                loss_batch += l1_lambda * l1_norm
