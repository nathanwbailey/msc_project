...
from model_decoder import SIMCLRDecoder, SIMCLR, Discriminator
...
def main():
    ...
    discriminator = Discriminator(in_channels=C)
    discriminator = discriminator.to(DEVICE)

    optimizer = torch.optim.Adam(model_decoder.parameters(), lr=learning_rate_decoder, weight_decay=0)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=10, threshold=0.0001)
    print('Fine Tuning Both')
    discriminator_optimizer = torch.optim.Adam(discriminator.parameters(), lr= 1e-5, weight_decay=0)


    train_encoder_decoder(model=model_decoder, discriminator=discriminator, num_epochs=num_epochs, trainloader=trainloader, testloader=validloader, optimizer=optimizer, discriminator_optimizer=discriminator_optimizer, scheduler=scheduler, device=DEVICE, loss_fn_contrastive=loss_fn_contrastive, loss_fn_reconstruct=loss_fn_reconstruct, cycle_loss=cycle_loss, gan_loss=gan_loss, model_save_path='simclr_decoder.pth', alpha=0.1)

    # print('Starting Downstream Task')
    downstream_task_lstm(num_epochs=100, data=test_data, encoder_model=model_decoder.model.encoder, latent_dim=1000, context_window=30, target_length=1, stride=1, model_save_path='downstream_model_no_decoder.pth')
    ...
    RECON_ALPHA = 0.5
    train_decoder(model=model_decoder, discriminator=discriminator, num_epochs=200, trainloader=trainloader, testloader=validloader, optimizer=optimizer, scheduler=scheduler, device=DEVICE, loss_fn_reconstruct=loss_fn_reconstruct, discriminator_optimizer=discriminator_optimizer, gan_loss=gan_loss, recon_alpha=RECON_ALPHA, model_save_path='simclr_decoder_freeze.pth')
