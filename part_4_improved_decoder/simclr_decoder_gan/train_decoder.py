...
def train_encoder_decoder(model, discriminator, num_epochs, trainloader, testloader, optimizer, discriminator_optimizer, scheduler, device, loss_fn_contrastive, loss_fn_reconstruct, cycle_loss, gan_loss, model_save_path="barlow_twins.pth", alpha=0.5):
    ...
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            d_real = discriminator(X)
            d_fake = discriminator(recon_masked)
            d_loss = (gan_loss(d_real, torch.ones_like(d_real)) + gan_loss(d_fake, torch.zeros_like(d_fake))) / 2
            discriminator_optimizer.zero_grad()
            d_loss.backward()
            discriminator_optimizer.step()
            ...
            d_fake = discriminator(recon_masked)
            gen_loss = gan_loss(d_fake, torch.ones_like(d_fake))

            loss_batch = alpha * loss_contrastive + (1-alpha) * (loss_recon_X + 0.5*gen_loss)
            loss_batch.backward()
            optimizer.step()
            ...
            discrim_train_loss.append(d_loss.item())
        ...
        print(
            ...
            f"  Train Loss     - Total: {np.mean(total_train_loss):.2f}, Contrastive: {np.mean(contrastive_train_loss):.2f}, Recon: {np.mean(recon_train_loss):.2f}, Generator: {np.mean(gen_train_loss):.2f}, Discriminator: {np.mean(discrim_train_loss):.2f}\n"
