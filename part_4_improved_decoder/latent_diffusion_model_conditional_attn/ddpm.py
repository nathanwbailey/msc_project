...
class DDPM(nn.Module):
    def __init__(self, eps_model, betas, n_T, loss_fn, device, *args, **kwargs):
    ...
    def forward(self, x, condition, t=None):
        if t is None:
            t = torch.randint(low=0, high=self.n_T+1, size=(x.shape[0],)).to(self.device)
    ...
    def sample(self, n_sample, size, condition, t = 0):
        ...
        for i in range(self.n_T, t, -1):
            time_tensor = (torch.full((n_sample, 1), i).float() / (self.n_T)).to(self.device)
            if i > 1:
                rand_noise = torch.randn_like(x_i)
            else:
                rand_noise = torch.zeros_like(x_i)
            rand_noise = rand_noise.to(self.device)
            x_i = self.oneover_sqrta[i]*(x_i - self.mab_over_sqrtmab[i]*self.eps_model(x_i, time_tensor, condition)) + self.sqrt_beta_t[i]*rand_noise
