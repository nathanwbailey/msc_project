...
def train_encoder_decoder(model, num_epochs, trainloader, testloader, optimizer, scheduler, device, loss_fn_contrastive, loss_fn_reconstruct, cycle_loss, model_save_path="barlow_twins.pth", alpha=0.5):
    ...
    for epoch in range(num_epochs):
        ...
        for data in trainloader:
            ...
            z2, _, z2_x =  model(X_prime_augment)
            Z_prime_2, _, Z_prime_2_X = model(X_prime_2)
            _, recon_masked, _ = model(X_masked)
            loss_cycle = cycle_loss(z2 - 2 * z1 + Z_prime_2, torch.zeros_like(z1))

            var_loss = variance_loss(z1_x) + variance_loss(z2_x) + variance_loss(Z_prime_2_X)
            cov_loss = covariance_loss(z1_x) + covariance_loss(z2_x) + covariance_loss(Z_prime_2_X)


            loss_contrastive = loss_fn_contrastive(z1, z2) + loss_fn_contrastive(z1, Z_prime_2) + loss_cycle + 1 * var_loss + 1 * cov_loss
        ...
        torch.save(model, f"/vol/bitbucket/nb324/masters_project/simclr_test_var2/simclr_decoder_{epoch}.pth")
        lr = optimizer.param_groups[0]['lr']
