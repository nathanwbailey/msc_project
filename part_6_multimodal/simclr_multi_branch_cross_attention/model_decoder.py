...
class SIMCLR(nn.Module):
    ...
    def __init__(self, in_channels, latent_dim):
        ...
        self.fuse_u_wind = nn.MultiheadAttention(embed_dim=1000, num_heads=1, batch_first=True)
        self.fuse_v_wind = nn.MultiheadAttention(embed_dim=1000, num_heads=1, batch_first=True)
        self.fuse_temp = nn.MultiheadAttention(embed_dim=1000, num_heads=1, batch_first=True)
        self.fuse_geo = nn.MultiheadAttention(embed_dim=1000, num_heads=1, batch_first=True)
    ...
    def encode(self, x):
        ...
        temp_out = self.fuse_temp(query=z_values[:, 0, :].unsqueeze(1), key=z_values[:, 3, :].unsqueeze(1), value=z_values[:, 3, :].unsqueeze(1))[0].squeeze(1)
        geo_out = self.fuse_geo(query=z_values[:, 3, :].unsqueeze(1), key=z_values[:, 0, :].unsqueeze(1), value=z_values[:, 0, :].unsqueeze(1))[0].squeeze(1)
        u_wind_out = self.fuse_u_wind(query=z_values[:, 1, :].unsqueeze(1), key=z_values[:, 2, :].unsqueeze(1), value=z_values[:, 2, :].unsqueeze(1))[0].squeeze(1)
        v_wind_out  = self.fuse_v_wind(query=z_values[:, 2, :].unsqueeze(1), key=z_values[:, 1, :].unsqueeze(1), value=z_values[:, 1, :].unsqueeze(1))[0].squeeze(1)
        attn_z_values = torch.stack([temp_out, u_wind_out, v_wind_out, geo_out, z_values[:, 4, :]], dim=1)
        out = attn_z_values.mean(dim=1)
        return out
